...
import dev.mfazio.wwc2019.types.firebase.FirebaseRanking
import dev.mfazio.wwc2019.types.firebase.FirebaseSPIRanking
...
class RankingService(@Autowired val teams: List<Team>, googleCredentials: GoogleCredentials) :
    ...
    fun loadRankings() {
        ...
        val rankings: Map<String, Ranking> = this.teams.associate { it.id to Ranking(
            it.name,
            fifa[it.id],
            elo[it.id],
            spi[it.id]
